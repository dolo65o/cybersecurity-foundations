...
- Scalability: effortless addition of new devices.
- Trade-off: higher cost due to required cabling and dedicated equipment.

## Bus Topology

This type of connection relies upon a single connection which is known as a backbone cable. This type of topology is similar to the leaf off of a tree in the sense that devices (leaves) stem from where the branches are on this cable.

- Bottlenecks: Single cable design causes latency during simultaneous data requests.
- Maintenance: Troubleshooting is complex due to the difficulty of isolating faults on the shared route.

## Ring Topology

The ring topology (also known as token topology) boasts some similarities. Devices such as computers are connected directly to each other to form a loop, meaning that there is little cabling required and less dependence on dedicated hardware such as within a star topology.

- Ring Architecture: Data travels sequentially across the loop.
- Priority Forwarding: Devices prioritize sending their own data before relaying traffic from other nodes.
